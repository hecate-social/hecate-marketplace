%%% @doc Path utilities for hecate-marketplaced.
%%%
%%% Provides purpose-specific path functions for the namespaced
%%% directory layout under ~/.hecate/hecate-marketplaced/:
%%%
...
%%% {hecate_marketplaced, [{data_dir, Path}]}.
%%% Default: ~/.hecate/hecate-marketplaced
%%% @end
-module(marketplaced_paths).
...
base_dir() ->
    case application:get_env(hecate_marketplaced, data_dir) of
        {ok, Dir} -> expand_path(Dir);
        undefined -> expand_path("~/.hecate/hecate-marketplaced")
    end.
...
gitops_apps_dir() ->
    Home = os:getenv("HOME"),
    filename:join([Home, ".hecate", "gitops", "apps"]).
